# ============================================================================
# CLAWG Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm https://raw.githubusercontent.com/NousResearch/clawg/main/scripts/install.ps1 | iex
#
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$ClawgHome = "$env:LOCALAPPDATA\clawg",
    [string]$InstallDir = "$env:LOCALAPPDATA\clawg\clawg"
)
...
$RepoUrlSsh = "git@github.com:NousResearch/clawg.git"
$RepoUrlHttps = "https://github.com/NousResearch/clawg.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ CLAWG Installer                   │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Test-Node {
    ...
    $managedNode = "$ClawgHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$ClawgHome\node;$env:Path"
        Write-Success "Node.js $version found (Clawg-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.clawg/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\clawg-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ClawgHome\node") { Remove-Item -Recurse -Force "$ClawgHome\node" }
                Move-Item $extractedDir.FullName "$ClawgHome\node"
                $env:Path = "$ClawgHome\node;$env:Path"

                $version = & "$ClawgHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.clawg/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/NousResearch/clawg/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\clawg-$Branch.zip"
                $extractPath = "$env:TEMP\clawg-extract"
...
function Set-PathVariable {
    Write-Info "Setting up clawg command..."
    
    if ($NoVenv) {
        $clawgBin = "$InstallDir"
    } else {
        $clawgBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so clawg is globally available
    # On Windows, the clawg.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$clawgBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$clawgBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $clawgBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set CLAWG_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\clawg instead
    # of the Unix default ~/.clawg
    $currentClawgHome = [Environment]::GetEnvironmentVariable("CLAWG_HOME", "User")
    if (-not $currentClawgHome -or $currentClawgHome -ne $ClawgHome) {
        [Environment]::SetEnvironmentVariable("CLAWG_HOME", $ClawgHome, "User")
        Write-Success "Set CLAWG_HOME=$ClawgHome"
    }
    $env:CLAWG_HOME = $ClawgHome
    
    # Update current session
    $env:Path = "$clawgBin;$env:Path"
    
    Write-Success "clawg command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.clawg directory structure
    New-Item -ItemType Directory -Force -Path "$ClawgHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawgHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawgHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawgHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawgHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawgHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawgHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawgHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawgHome\skills" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawgHome\whatsapp\session" | Out-Null
    
    # Create .env
    $envPath = "$ClawgHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.clawg/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.clawg/.env"
        }
    } else {
        Write-Info "~/.clawg/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$ClawgHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.clawg/config.yaml from template"
        }
    } else {
        Write-Info "~/.clawg/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$ClawgHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.clawg/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.clawg/"
    
    # Seed bundled skills into ~/.clawg/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.clawg/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.clawg/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ClawgHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.clawg/skills/"
            }
        }
    }
}
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m clawg_cli.main setup
    } else {
        python -m clawg_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $clawgCmd = "$InstallDir\venv\Scripts\clawg.exe"
    if (-not (Test-Path $clawgCmd)) {
        $clawgCmd = "clawg"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ClawgHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'clawg whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ClawgHome\logs\gateway.log"
            Start-Process -FilePath $clawgCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ClawgHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: clawg gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: clawg gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ClawgHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ClawgHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ClawgHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ClawgHome\clawg\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   clawg              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   clawg setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   clawg config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   clawg config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   clawg gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   clawg update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/clawg/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
