...
description: Migrate a user's OpenClaw customization footprint into CLAWG. Imports Clawg-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: CLAWG (Nous Research)
license: MIT
metadata:
  clawg:
    tags: [Migration, OpenClaw, clawg, Memory, Persona, Import]
    related_skills: [clawg]
---

# OpenClaw -> clawg Migration

Use this skill when a user wants to move their OpenClaw setup into CLAWG with minimal manual cleanup.
...
```bash
clawg claw migrate              # Full interactive migration
clawg claw migrate --dry-run    # Preview what would be migrated
clawg claw migrate --preset user-data   # Migrate without secrets
clawg claw migrate --overwrite  # Overwrite existing conflicts
clawg claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `clawg setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_clawg.py` to:

- import `SOUL.md` into the clawg home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into clawg memory entries
- merge OpenClaw command approval patterns into clawg `command_allowlist`
- migrate Clawg-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.clawg/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen clawg workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.clawg/tts/`
- archive non-secret docs that do not have a direct clawg destination
...
- `scripts/openclaw_to_clawg.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.clawg/skills/migration/openclaw-migration/scripts/openclaw_to_clawg.py`

Do not guess a shorter path like `~/.clawg/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.clawg/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Clawg-compatible secrets, currently:
...
clawg CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in clawg. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a clawg workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing clawg skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.clawg/skills/migration/openclaw-migration/scripts/openclaw_to_clawg.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.clawg/skills/migration/openclaw-migration/scripts/openclaw_to_clawg.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.clawg/skills/migration/openclaw-migration/scripts/openclaw_to_clawg.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.clawg/skills/migration/openclaw-migration/scripts/openclaw_to_clawg.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.clawg/skills/migration/openclaw-migration/scripts/openclaw_to_clawg.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.clawg/skills/migration/openclaw-migration/scripts/openclaw_to_clawg.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of clawg unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty clawg targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean clawg destination. Unsupported auth blobs must still be reported as skipped.
...
- clawg persona state imported
- clawg memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.clawg/skills/openclaw-imports/`
