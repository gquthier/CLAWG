...
version: 1.0.0
author: arceus77-7, enhanced by CLAWG
license: MIT
metadata:
  clawg:
...
- `tmux` available for stable authenticated sessions during clawg terminal calls (desktop app flow only)
...
### Service Account (recommended for clawg)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.clawg/.env` (the skill will prompt for this on first load).
...
## clawg Execution Pattern (desktop app flow)

clawg terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/clawg-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/clawg-op.sock"
