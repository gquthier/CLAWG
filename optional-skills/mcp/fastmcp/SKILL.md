...
- smoke-test a server with the FastMCP CLI before wiring it into clawg or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to clawg. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.clawg/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.clawg/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is clawg integration, either:

- configure the server in `~/.clawg/config.yaml` using the `native-mcp` skill, or
...
### clawg cannot see the deployed server

The server-building part may be correct while the clawg config is not. Load the `native-mcp` skill and configure the server in `~/.clawg/config.yaml`, then restart clawg.
