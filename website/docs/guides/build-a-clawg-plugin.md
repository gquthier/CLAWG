...
# Build a clawg Plugin

This guide walks through building a complete clawg plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.
...
```bash
mkdir -p ~/.clawg/plugins/calculator
cd ~/.clawg/plugins/calculator
```
...
This tells clawg: "I'm a plugin called calculator, I provide tools and hooks." That's all the manifest needs.
...
4. **Accept `**kwargs`:** clawg may pass additional context in the future.
...
- If this function crashes, the plugin is disabled but clawg continues fine
...
def _install_skill():
    """Copy our skill to ~/.clawg/skills/ on first load."""
    try:
        from clawg_cli.config import get_clawg_home
        dest = get_clawg_home() / "skills" / "my-plugin" / "SKILL.md"
    except Exception:
        dest = Path.home() / ".clawg" / "skills" / "my-plugin" / "SKILL.md"
...
```toml
# pyproject.toml
[project.entry-points."clawg_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install clawg-plugin-calculator
# Plugin auto-discovered on next clawg startup
```
...
```python
# Wrong — will break if clawg passes extra context
def handler(args):
    ...
