---
sidebar_position: 7
title: "Use Voice Mode with clawg"
description: "A practical guide to setting up and using clawg voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with clawg
...
- you want clawg sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in clawg.
...
## Step 1: make sure normal clawg works first
...
clawg supports both local and cloud speech stacks.
...
### If you use `clawg setup`

If you choose NeuTTS in the setup wizard, clawg checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. clawg transcribes and responds
...
- asking clawg to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full clawg loop.

## Tuning CLI behavior

### Silence threshold

If clawg starts/stops too aggressively, tune:
...
clawg stays a normal chat bot, but can speak replies.
...
- you want clawg to function like a portable research or ops assistant
...
clawg joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- clawg detects speech boundaries
- transcripts are posted in the associated text channel
- clawg responds in text and audio
...
1. get text clawg working
2. install `clawg[voice]`
