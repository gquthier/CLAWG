...
This tutorial walks you through setting up a Telegram bot powered by CLAWG that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **CLAWG installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/learning-path) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.clawg/.env`

:::tip
A $5/month VPS is plenty for running the gateway. clawg itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team clawg Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_clawg_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by CLAWG. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting clawg Gateway...
...
# View live logs
journalctl --user -u clawg-gateway -f
...
```bash
clawg gateway stop && clawg gateway start
```
...
clawg pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.clawg/.env`:
...
Control how much detail the bot shows when using tools. In `~/.clawg/config.yaml`:
...
Customize how the bot communicates by editing `~/.clawg/SOUL.md`:

For a full guide, see [Use SOUL.md with clawg](/docs/guides/use-soul-with-clawg).
...
```bash
# From the CLI
clawg cron list          # View all scheduled jobs
clawg cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u clawg-gateway` (Linux) or `~/.clawg/logs/gateway.log` (macOS) |
| Cron job output | `~/.clawg/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.clawg/cron/jobs.json` |
| Pairing data | `~/.clawg/pairing/` |
| Session history | `~/.clawg/sessions/` |
