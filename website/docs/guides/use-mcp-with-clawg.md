---
sidebar_position: 5
title: "Use MCP with clawg"
description: "A practical guide to connecting MCP servers to CLAWG, filtering their tools, and using them safely in real workflows"
---

# Use MCP with clawg

This guide shows how to actually use MCP with CLAWG in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native clawg tool
- you want clawg to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect clawg to internal APIs, databases, or company systems without modifying clawg core

Do not use MCP when:
- a built-in clawg tool already solves the job well
...
- clawg remains the agent
- MCP servers contribute tools
- clawg discovers those tools at startup or reload time
...
If you installed clawg with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- clawg banner/status should show MCP integration when configured
- ask clawg what tools it has available
...
There are two categories of MCP-exposed functionality in clawg:
...
2. Clawg-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So clawg will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want clawg to reason over a bounded workspace.
...
Now clawg can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing clawg core.
...
Because clawg now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
