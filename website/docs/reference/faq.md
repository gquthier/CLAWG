...
description: "Frequently asked questions and solutions to common issues with CLAWG"
---
...
### What LLM providers work with clawg?

CLAWG works with any OpenAI-compatible API. Supported providers include:
...
Set your provider with `clawg model` or by editing `~/.clawg/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** CLAWG requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run clawg from inside it. The standard install command works perfectly in WSL2:
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). CLAWG does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.clawg/`.

### Can I use it offline / with local models?

Yes. Run `clawg model`, select **Custom endpoint**, and enter your server's URL:
...
clawg persists the endpoint in `config.yaml` and prompts for the context window size so compression triggers at the right time. If you leave context length blank, clawg auto-detects it from the server's `/models` endpoint or [models.dev](https://models.dev).
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in clawg — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

### How much does it cost?

CLAWG itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same CLAWG instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use clawg programmatically:

```python
from clawg.agent import AIAgent

agent = AIAgent(model="openrouter/nous/clawg-3-llama-3.1-70b")
...
#### `clawg: command not found` after installation
...
**Cause:** clawg requires Python 3.11 or newer.
...
clawg config get OPENROUTER_API_KEY

# Re-configure your provider
clawg model

# Or set directly
clawg config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.clawg/.env` for conflicting entries.
:::
...
# Set a valid model
clawg config set CLAWG_MODEL openrouter/nous/clawg-3-llama-3.1-70b

# Or specify per-session
clawg chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `clawg chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or clawg detected the wrong context length for your model.
...
clawg chat --model openrouter/google/gemini-2.0-flash-001
```

If this happens on the first long conversation, clawg may have the wrong context length for your model. Check what it detected:
...
**Cause:** clawg detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — clawg never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `clawg chat`
...
**Solution:**
- Verify your bot token is valid with `clawg gateway setup`
- Check gateway logs: `clawg gateway logs`
...
Configure in `~/.clawg/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install clawg[telegram]   # or [discord], [slack], [whatsapp]
...
**Solution:**
- Try a faster/smaller model: `clawg chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `clawg chat -t "terminal"`
...
Verify your `~/.clawg/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
clawg config show | grep -A 12 mcp_servers

# Restart clawg or reload MCP after config changes
clawg chat
```
...
:::warning
If an MCP server crashes mid-request, clawg will report a timeout. Check the server's own logs (not just clawg logs) to diagnose the root cause.
:::
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/clawg/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), clawg version (`clawg --version`), and the full error message
