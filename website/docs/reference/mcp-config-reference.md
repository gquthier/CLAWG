...
description: "Reference for CLAWG MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
clawg may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, clawg only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, clawg does not create an empty MCP runtime toolset for that server.
