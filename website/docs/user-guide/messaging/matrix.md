...
sidebar_position: 9
title: "Matrix"
description: "Set up CLAWG as a Matrix bot"
---

# Matrix Setup

CLAWG integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `matrix-nio` Python SDK, processes messages through the CLAWG pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

clawg works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how clawg behaves once it's connected.
...
| **Rooms** | clawg responds to all messages in rooms it has joined. Room invites are auto-accepted. |
| **Threads** | clawg supports Matrix threads (MSC3440). If you reply in a thread, clawg keeps the thread context isolated from the main room timeline. |
| **Shared rooms with multiple users** | By default, clawg isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
2. Choose a username like `clawg` — the full user ID will be `@clawg:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `clawg-bot`).

### Option C: Use Your Own Account

You can also run clawg as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

clawg needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give clawg the bot's user ID and password. clawg will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@clawg:your-server.org
MATRIX_PASSWORD=your-password
```

## Step 3: Find Your Matrix User ID

CLAWG uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
# MATRIX_USER_ID=@clawg:matrix.example.org
...
clawg supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with clawg extras
pip install 'clawg[matrix]'
```
...
Add to your `~/.clawg/.env`:

```bash
MATRIX_ENCRYPTION=true
```

When E2EE is enabled, clawg:

- Stores encryption keys in `~/.clawg/matrix/store/`
...
:::warning
If you delete the `~/.clawg/matrix/store/` directory, the bot loses its encryption keys. You'll need to verify the device again in your Matrix client. Back up this directory if you want to preserve encrypted sessions.
:::
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the clawg logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The clawg gateway isn't running, or it failed to connect.

**Fix**: Check that `clawg gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.clawg/.env` and restart the gateway. Use the full `@user:server` format.
...
- **Media support**: clawg can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
