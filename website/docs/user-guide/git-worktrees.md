...
sidebar_position: 9
title: "Git Worktrees"
description: "Run multiple CLAWGs safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

CLAWG is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with clawg so each session has a clean, isolated working directory.

## Why Use Worktrees with clawg?

clawg treats the **current working directory** as the project root:

- CLI: the directory where you run `clawg` or `clawg chat`
...
git worktree add ../repo-feature feature/clawg-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/clawg-experiment` checked out in that directory

Now you can `cd` into the new worktree and run clawg there:

```bash
cd ../repo-feature

# Start clawg in the worktree
clawg
```
...
git worktree add ../repo-experiment-a feature/clawg-a
git worktree add ../repo-experiment-b feature/clawg-b
```
...
- Works on its own branch (`feature/clawg-a` vs `feature/clawg-b`).
...
- clawg checkpoint data under `~/.clawg/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per clawg experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/clawg-checkpoints-docs`, `feature/clawg-refactor-tests`.
- **Commit frequently**
...
- **Avoid running clawg from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `clawg -w` (Automatic Worktree Mode)

clawg has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `clawg/clawg-<hash>`).
...
```bash
clawg -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `clawg -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each clawg session its own clean checkout.
