...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with CLAWG — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

CLAWG supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with clawg](/docs/guides/use-voice-mode-with-clawg).

## Prerequisites

Before using voice features, make sure you have:

1. **CLAWG installed** — `pip install clawg` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `clawg model` or set your preferred provider credentials in `~/.clawg/.env`
3. **A working base setup** — run `clawg` to verify the agent responds to text before enabling voice

:::tip
The `~/.clawg/` directory and default `config.yaml` are created automatically the first time you run `clawg`. You only need to create `~/.clawg/.env` manually for API keys.
:::
...
pip install clawg[tts-premium]

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install clawg[all]
```
...
```bash
clawg                # Start the interactive CLI
```
...
1. Start the CLI with `clawg` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.clawg/config.yaml` (default: `ctrl+b`).
:::
...
```bash
clawg gateway        # Start the gateway (connects to configured platforms)
clawg gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@clawgbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.clawg/.env`:
```bash
...
```bash
clawg gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.clawg/.env`
...
- Review gateway logs: `tail -f ~/.clawg/logs/gateway.log`
