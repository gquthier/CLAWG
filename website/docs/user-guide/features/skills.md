...
All skills live in **`~/.clawg/skills/`** — a single directory that serves as the source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.
...
The bundled `plan` skill is a good example of a skill-backed slash command with custom behavior. Running `/plan [request]` tells clawg to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.clawg/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
clawg chat --toolsets skills -q "What skills do you have?"
clawg chat --toolsets skills -q "Show me the axolotl skill"
```
...
When a missing value is encountered, clawg asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `clawg setup` or `~/.clawg/.env` locally instead.

## Skill Directory Structure

```text
~/.clawg/skills/                  # Single source of truth
...
```bash
clawg skills browse                              # Browse all hub skills (official first)
clawg skills browse --source official            # Browse only official optional skills
clawg skills search kubernetes                   # Search all sources
clawg skills search react --source skills-sh     # Search the skills.sh directory
clawg skills search https://mintlify.com/docs --source well-known
clawg skills inspect openai/skills/k8s           # Preview before installing
clawg skills install openai/skills/k8s           # Install with security scan
...
clawg skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
clawg skills list --source hub                   # List hub-installed skills
clawg skills check                               # Check installed hub skills for upstream updates
clawg skills update                              # Reinstall hub skills with upstream changes when needed
clawg skills audit                               # Re-scan all hub skills for security
clawg skills uninstall k8s                       # Remove a hub skill
clawg skills publish skills/my-skill --to github --repo owner/repo
clawg skills snapshot export setup.json          # Export skill config
clawg skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with clawg. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `clawg skills search <query> --source skills-sh`. clawg resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
clawg currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the clawg repository itself and install with builtin trust.
...
This is Vercel's public skills directory. clawg can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
clawg skills search https://mintlify.com/docs --source well-known
clawg skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
clawg skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

clawg can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
clawg skills tap add myorg/skills-repo
```
...
- clawg source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

clawg supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

clawg source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

clawg can search and convert agent entries from LobeHub's public catalog into installable clawg skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- clawg source id: `lobehub`
...
`clawg skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with clawg | Always trusted |
...
```bash
clawg skills check          # Report which installed hub skills changed upstream
clawg skills update         # Reinstall only the skills with updates available
clawg skills update react   # Update one specific installed hub skill
```
