...
description: "Customize CLAWG's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

CLAWG's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `CLAWG_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who clawg is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

clawg now seeds a default `SOUL.md` automatically in:

```text
~/.clawg/SOUL.md
```

More precisely, it uses the current instance's `CLAWG_HOME`, so if you run clawg with a custom home directory, it will use:
...
- clawg creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- clawg loads `SOUL.md` only from `CLAWG_HOME`
- clawg does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, clawg falls back to a built-in default identity
...
If clawg loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `CLAWG_HOME`, the personality belongs to the clawg instance itself.

That also makes it easier to teach users:
- "Edit `~/.clawg/SOUL.md` to change clawg' default personality."
...
- how clawg should handle uncertainty, disagreement, or ambiguity
...
## What clawg injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, clawg falls back to a built-in default identity ("You are CLAWG, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
clawg ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain clawg, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives clawg its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.clawg/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.clawg/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-clawg)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how clawg speaks
- `display.skin` and `/skin` affect how clawg looks in the terminal
