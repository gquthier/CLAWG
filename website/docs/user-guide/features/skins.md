...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the clawg CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the clawg CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.clawg/skins/mytheme.yaml
```

Or set the default skin in `~/.clawg/config.yaml`:
...
|------|-------------|----------------|
| `default` | Classic clawg — gold and kawaii | `CLAWG` |
| `ares` | War-god theme — crimson and bronze | `Ares Agent` |
| `mono` | Monochrome — clean grayscale | `CLAWG` |
| `slate` | Cool blue — developer-focused | `CLAWG` |
...
Create YAML files under `~/.clawg/skins/`. User skins inherit missing values from the built-in `default` skin.
...
- Built-in skins load from `clawg_cli/skin_engine.py`.
