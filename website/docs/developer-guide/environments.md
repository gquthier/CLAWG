...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the clawg Atropos integration"
---

# Environments, Benchmarks & Data Generation

CLAWG includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for clawg/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- ClawgAgentBaseEnv
    ClawgAgentBaseEnv <|-- TerminalTestEnv
    ClawgAgentBaseEnv <|-- ClawgSweEnv
    ClawgAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The clawg layer (`environments/clawg_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls clawg's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `ClawgAgentLoop` and scores the result
...
Your environment inherits from `ClawgAgentBaseEnv` and implements five methods:
...
`ClawgAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as clawg's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any clawg tool |
...
parser = get_parser("clawg")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `clawg`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
```bash
python environments/clawg_swe_env/clawg_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"clawg"`, `"qwen"`, `"mistral"`)
...
```python
from environments.clawg_base_env import ClawgAgentBaseEnv, ClawgAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(ClawgAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(ClawgAgentBaseEnv):
    name = "my-env"
...
### ClawgAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which clawg toolsets to enable |
...
| `tool_call_parser` | `str` | `"clawg"` | Parser for Phase 2 raw output |
...
env:
  ...
  tokenizer_name: "NousResearch/Clawg-3-Llama-3.1-8B"
  use_wandb: true
...
- [Modal](https://modal.com) account and CLI: `pip install "clawg[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "clawg[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── clawg_base_env.py          # Abstract base class (ClawgAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (ClawgAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── clawg_parser.py        # clawg/ChatML <tool_call> format
...
├── clawg_swe_env/             # SWE-bench training environment
│
