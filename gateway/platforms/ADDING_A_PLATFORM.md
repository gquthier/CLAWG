...
Checklist for integrating a new messaging platform into the clawg gateway.
...
And add it to the `clawg-gateway` composite:

```python
"clawg-gateway": {
    "includes": [..., "clawg-your-platform"]
}
```
...
## 12. Status Display (`clawg_cli/status.py`)
...
## 13. Gateway Setup Wizard (`clawg_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ clawg_cli/ toolsets.py \
  --include="*.py" -l | sort -u
