...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_clawg_home() / "sessions")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        sessions_dir = get_clawg_home() / "sessions"
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.clawg/config.yaml (primary user-facing config)
    3. ~/.clawg/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_clawg_home()
    gw_data: dict = {}
