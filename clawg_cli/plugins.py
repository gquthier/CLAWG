...
1. **User plugins**   – ``~/.clawg/plugins/<name>/``
2. **Project plugins** – ``./.clawg/plugins/<name>/``
3. **Pip plugins**     – packages that expose the ``clawg_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "clawg_agent.plugins"

_NS_PARENT = "clawg_plugins"
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.clawg/plugins/)
        clawg_home = os.environ.get("CLAWG_HOME", os.path.expanduser("~/.clawg"))
        user_dir = Path(clawg_home) / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.clawg/plugins/)
        project_dir = Path.cwd() / ".clawg" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``clawg_plugins.<name>``."""
