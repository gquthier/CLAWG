...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into CLAWG_HOME if the user doesn't have one yet."""
...
OPTIONAL_ENV_VARS = {
    ...
    # CLAWG_TOOL_PROGRESS and CLAWG_TOOL_PROGRESS_MODE are deprecated —
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("CLAWG_TOOL_PROGRESS")
            old_mode = get_env_value("CLAWG_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from CLAWG_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from CLAWG_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("CLAWG_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from CLAWG_TIMEZONE)")
            else:
                config["timezone"] = ""
