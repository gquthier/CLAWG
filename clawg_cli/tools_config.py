"""
Unified tool configuration for CLAWG.

`clawg tools` and `clawg setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.clawg/config.yaml under
...
# They're still in _CLAWG_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "rl"}

# Platform display config
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "clawg-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "clawg-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "clawg-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "clawg-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "clawg-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "clawg-signal"},
    "email":    {"label": "📧 Email",      "default_toolset": "clawg-email"},
    "dingtalk": {"label": "💬 DingTalk",   "default_toolset": "clawg-dingtalk"},
}
    ...
    if post_setup_key == "browserbase":
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning("    npm install failed - run manually: cd ~/.clawg/clawg && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in clawg directory)")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `clawg tools` and `clawg setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ clawg Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    print(color("  Tool configuration saved to ~/.clawg/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'clawg' or gateway restart.", Colors.DIM))
    print()
