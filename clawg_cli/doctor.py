"""
Doctor command for clawg CLI.

Diagnoses issues with CLAWG setup.
"""
...
from clawg_cli.config import get_project_root, get_clawg_home, get_env_path

PROJECT_ROOT = get_project_root()
CLAWG_HOME = get_clawg_home()

# Load environment variables from ~/.clawg/.env so API key checks work
...
# Point mini-swe-agent at ~/.clawg/ so it shares our config
os.environ.setdefault("MSWEA_GLOBAL_CONFIG_DIR", str(CLAWG_HOME))
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.clawg/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `clawg`.
    os.environ.setdefault("CLAWG_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 clawg Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.clawg/.env (primary location for user config)
    env_path = CLAWG_HOME / '.env'
    if env_path.exists():
        check_ok("~/.clawg/.env file exists")
        ...
        if _has_provider_env_config(content):
            ...
            check_warn("No API key found in ~/.clawg/.env")
            issues.append("Run 'clawg setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            check_fail("~/.clawg/.env file missing")
            if should_fix:
                env_path.parent.mkdir(parents=True, exist_ok=True)
                env_path.touch()
                check_ok("Created empty ~/.clawg/.env")
                check_info("Run 'clawg setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'clawg setup' to create one")
                issues.append("Run 'clawg setup' to create .env")
    
    # Check ~/.clawg/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = CLAWG_HOME / 'config.yaml'
    if config_path.exists():
        check_ok("~/.clawg/config.yaml exists")
    else:
        ...
        if fallback_config.exists():
            ...
            if should_fix and example_config.exists():
                ...
                check_ok("Created ~/.clawg/config.yaml from cli-config.yaml.example")
                fixed_count += 1
            elif should_fix:
                check_warn("config.yaml not found and no example to copy from")
                manual_issues.append("Create ~/.clawg/config.yaml manually")
            else:
    ...
    try:
        from clawg_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    clawg_home = CLAWG_HOME
    if clawg_home.exists():
        check_ok("~/.clawg directory exists")
    else:
        if should_fix:
            clawg_home.mkdir(parents=True, exist_ok=True)
            check_ok("Created ~/.clawg directory")
            fixed_count += 1
        else:
            check_warn("~/.clawg not found", "(will be created on first use)")
    
    # Check expected subdirectories
    expected_subdirs = ["cron", "sessions", "logs", "skills", "memories"]
    for subdir_name in expected_subdirs:
        subdir_path = clawg_home / subdir_name
        if subdir_path.exists():
            check_ok(f"~/.clawg/{subdir_name}/ exists")
        else:
            if should_fix:
                subdir_path.mkdir(parents=True, exist_ok=True)
                check_ok(f"Created ~/.clawg/{subdir_name}/")
                fixed_count += 1
            else:
                check_warn(f"~/.clawg/{subdir_name}/ not found", "(will be created on first use)")
    
    # Check for SOUL.md persona file
    soul_path = clawg_home / "SOUL.md"
    if soul_path.exists():
        content = soul_path.read_text(encoding="utf-8").strip()
        # Check if it's just the template comments (no real content)
        lines = [l for l in content.splitlines() if l.strip() and not l.strip().startswith(("<!--", "-->", "#"))]
        if lines:
            check_ok("~/.clawg/SOUL.md exists (persona configured)")
        else:
            check_info("~/.clawg/SOUL.md exists but is empty — edit it to customize personality")
    else:
        check_warn("~/.clawg/SOUL.md not found", "(create it to give clawg a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# CLAWG Persona\n\n"
                "<!-- Edit this file to customize how clawg communicates. -->\n\n"
                "You are clawg, a helpful AI assistant.\n",
                encoding="utf-8",
            )
            check_ok("Created ~/.clawg/SOUL.md with basic template")
            fixed_count += 1
    
    # Check memory directory
    memories_dir = clawg_home / "memories"
    if memories_dir.exists():
        check_ok("~/.clawg/memories/ directory exists")
        ...
        check_warn("~/.clawg/memories/ not found", "(will be created on first use)")
        if should_fix:
            memories_dir.mkdir(parents=True, exist_ok=True)
            check_ok("Created ~/.clawg/memories/")
            fixed_count += 1
    
    # Check SQLite session store
    state_db_path = clawg_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            conn.close()
            check_ok(f"~/.clawg/state.db exists ({count} sessions)")
        except Exception as e:
            check_warn(f"~/.clawg/state.db exists but has issues: {e}")
    else:
        check_info("~/.clawg/state.db not created yet (will be created on first session)")
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint — https://github.com/NousResearch/clawg/issues/811
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'clawg setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: clawg skills list)")
    ...
    if github_token:
        ...
        check_warn("No GITHUB_TOKEN", "(60 req/hr rate limit — set in ~/.clawg/.env for better rates)")
    ...
    try:
        ...
        if not GLOBAL_CONFIG_PATH.exists():
            check_warn("Honcho config not found", f"run: clawg honcho setup")
        elif not hcfg.enabled:
            check_info("Honcho disabled (set enabled: true in ~/.honcho/config.json to activate)")
        elif not hcfg.api_key:
            check_fail("Honcho API key not set", "run: clawg honcho setup")
            issues.append("No Honcho API key — run 'clawg honcho setup'")
        else:
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'clawg doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
