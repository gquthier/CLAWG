"""
Interactive setup wizard for CLAWG.
...
Config files are stored in ~/.clawg/ for easy access.
"""
...
def _setup_copilot_reasoning_selection(
    ...
    *,
    ...
    from clawg_cli.models import github_model_reasoning_efforts, normalize_copilot_model_id
...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    from clawg_cli.config import get_env_value
...
from clawg_cli.colors import Colors, color
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ clawg Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure clawg using environment variables or config commands:")
    print_info("  clawg config set model.provider custom")
    print_info("  clawg config set model.base_url http://localhost:8080/v1")
    print_info("  clawg config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'clawg setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from clawg_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning(f"  Skipped (configure later with 'clawg setup')")


def _print_setup_summary(config: dict, clawg_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'clawg setup' to configure"))
    ...
    if tts_provider == "elevenlabs" and get_env_value("ELEVENLABS_API_KEY"):
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'clawg setup tts'"))
    else:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'clawg setup tools' to configure them,"
        )
        print_warning("or edit ~/.clawg/.env directly to add the missing API keys.")
        print()
    ...
    print(color("📁 All your files are in ~/.clawg/:", Colors.CYAN, Colors.BOLD))
    print()
    print(f"   {color('Settings:', Colors.YELLOW)}  {get_config_path()}")
    print(f"   {color('API Keys:', Colors.YELLOW)}  {get_env_path()}")
    print(
        f"   {color('Data:', Colors.YELLOW)}      {clawg_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('clawg setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('clawg setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('clawg setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('clawg setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('clawg setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('clawg config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('clawg config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('clawg config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('clawg', Colors.GREEN)}              Start chatting")
    print(f"   {color('clawg gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('clawg doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `clawg tools` and `clawg setup tools`).
...
def setup_model_provider(config: dict):
    ...
    if not has_any_provider:
        print_warning("An inference provider is required for clawg to work.")
        print()
    ...
    if provider_idx == 0:  # Nous Portal (OAuth)
        ...
        try:
            from clawg_cli.auth import _login_nous, ProviderConfig
            import argparse
            ...
            print_info("You can try again later with: clawg model")
        ...
        try:
            from clawg_cli.auth import deactivate_provider
        ...
        config_path = (
            Path(os.environ.get("CLAWG_HOME", Path.home() / ".clawg")) / "config.yaml"
        )
        try:
            disk_cfg = {}
        ...
        # Reuse the shared custom endpoint flow from `clawg model`.
        ...
        from clawg_cli.main import _model_flow_custom
        ...
        if api_key:
            ...
            from clawg_cli.auth import detect_zai_endpoint
        ...
        from clawg_cli.config import save_anthropic_api_key, save_anthropic_oauth_token
        ...
        print_info("clawg can use GITHUB_TOKEN, GH_TOKEN, or your gh CLI login.")
        print_info(f"Base URL: {pconfig.inference_base_url}")
        print()
        ...
        print_info("clawg will start `copilot --acp --stdio` for each request.")
        print_info("Use CLAWG_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH to override the command.")
    ...
    if selected_provider in _vision_backends:
        # If the user just selected a backend clawg can already use for
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one clawg can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'clawg setup' or configure AUXILIARY_VISION_* settings")
    ...
    if selected_provider != "custom":  # Custom already prompted for model name
        ...
        if selected_provider == "nous" and nous_models:
            ...
            from clawg_cli.models import provider_model_ids
            ...
            from clawg_cli.models import model_ids, menu_labels
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'clawg setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where clawg runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using clawg via Telegram/Discord, this is where")
        print_info(
...
def setup_agent_settings(config: dict):
    ...
    current_max = get_env_value("CLAWG_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("CLAWG_MAX_ITERATIONS", str(max_iter))
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with clawg from anywhere.")
    print()
    ...
    if not existing_telegram and prompt_yes_no("Set up Telegram bot?", False):
        ...
        if token:
            ...
            print()
            print_info("📬 Home Channel: where clawg delivers cron job results,")
    ...
    if not existing_slack and prompt_yes_no("Set up Slack bot?", False):
        ...
        print()
        print_info(
            "   Full guide: https://clawg.nousresearch.com/docs/user-guide/messaging/slack/"
        )
        print()
    ...
    if not existing_matrix and prompt_yes_no("Set up Matrix?", False):
        ...
        if token or get_env_value("MATRIX_PASSWORD"):
            ...
            # Home room
            print()
            print_info("📬 Home Room: where clawg delivers cron job results and notifications.")
    ...
    if not existing_mattermost and prompt_yes_no("Set up Mattermost?", False):
        ...
        if token:
            ...
            # Home channel
            print()
            print_info("📬 Home Channel: where clawg delivers cron job results and notifications.")
    ...
    if not existing_whatsapp and prompt_yes_no("Set up WhatsApp?", False):
        ...
        print_info("Requires Node.js. Run 'clawg whatsapp' for guided setup.")
        print()
        if prompt_yes_no("Enable WhatsApp now?", True):
            save_env_value("WHATSAPP_ENABLED", "true")
            print_success("WhatsApp enabled")
            print_info("Run 'clawg whatsapp' to choose your mode (separate bot number")
    ...
    if not existing_webhook and prompt_yes_no("Set up webhooks? (GitHub, GitLab, etc.)", False):
        ...
        print()
        print_info(
            "   Full guide: https://clawg.nousresearch.com/docs/user-guide/messaging/webhooks/"
        )
        print()
        ...
        print_info("   1. Define webhook routes in ~/.clawg/config.yaml")
        ...
        print_info(
            "   https://clawg.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes"
        )
        print()
        print_info("   Open config in your editor:  clawg config edit")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     clawg config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: clawg gateway install")
            else:
                print_info("  You can install later: clawg gateway install")
                if _is_linux:
                    print_info("  Or as a boot-time service: sudo clawg gateway install --system")
                print_info("  Or run in foreground:  clawg gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   clawg gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `clawg setup tools` and `clawg tools` use the same flow:
    ...
    """
    from clawg_cli.tools_config import tools_command
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_clawg.py"
)


def _offer_openclaw_migration(clawg_home: Path) -> bool:
    ...
    print_info("clawg can import your settings, memories, skills, and API keys.")
    print()
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "openclaw_to_clawg", _OPENCLAW_SCRIPT
        )
        ...
        migrator = mod.Migrator(
            ...
            target_root=clawg_home.resolve(),
            execute=True,
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in clawg.")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      clawg setup           — full or quick (auto-detected)
      clawg setup model     — just model/provider
      clawg setup terminal  — just terminal backend
      clawg setup gateway   — just messaging platforms
      clawg setup tools     — just tool configuration
      clawg setup agent     — just agent settings
    """
    ensure_clawg_home()

    config = load_config()
    clawg_home = get_clawg_home()
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ clawg Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    from clawg_cli.auth import get_active_provider
    ...
    print(
        color(
            "│             ⚕ CLAWG Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your CLAWG installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have clawg configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, clawg_home)
            return
        elif choice == 1:
            ...
            print_info("Exiting. Run 'clawg setup' again when ready.")
            return
        elif choice == 9:
            print_info("Exiting. Run 'clawg setup' again when ready.")
            return
            ...
            func(config)
            save_config(config)
            _print_setup_summary(config, clawg_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        if _offer_openclaw_migration(clawg_home):
    ...
    print_info(f"Data folder:  {clawg_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'clawg config edit'")
...
def _run_quick_setup(config: dict, clawg_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'clawg setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect clawg to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'clawg setup gateway'.")
