"""clawg claw — OpenClaw migration commands.

Usage:
    clawg claw migrate              # Interactive migration from ~/.openclaw
    clawg claw migrate --dry-run    # Preview what would be migrated
    clawg claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
"""
...
from clawg_cli.config import get_clawg_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_clawg.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_clawg", script_path)
...
def claw_command(args):
    """Route clawg claw subcommands."""
    action = getattr(args, "claw_action", None)

    if action == "migrate":
        _cmd_migrate(args)
    else:
        print("Usage: clawg claw migrate [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to clawg")
        print()
        print("Run 'clawg claw migrate --help' for migration options.")


def _cmd_migrate(args):
    """Run the OpenClaw → clawg migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info(f"You can specify a custom path: clawg claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {clawg_home}")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  clawg claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  clawg claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  clawg config set OPENROUTER_API_KEY sk-or-v1-...")
