...
from clawg_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from clawg_cli.config import get_env_path, get_clawg_home
    from clawg_cli.auth import get_auth_status
...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        ...
        from clawg_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
    ...
    try:
        from clawg_cli.banner import prefetch_update_check
...
def cmd_gateway(args):
    ...
    from clawg_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from clawg_cli.config import get_env_value, save_env_value
    ...
    session_dir = get_clawg_home() / "whatsapp" / "session"
...
def cmd_setup(args):
    ...
    from clawg_cli.setup import run_setup_wizard
...
def cmd_model(args):
    ...
    from clawg_cli.config import load_config, save_config, get_env_value, save_env_value
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from clawg_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from clawg_cli.config import get_env_value, save_env_value
    ...
    from clawg_cli.models import model_ids
...
def _model_flow_custom(config):
    ...
    """
    from clawg_cli.auth import _save_model_choice, deactivate_provider
    from clawg_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    from clawg_cli.models import probe_api_models
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from clawg_cli.auth import _save_model_choice, deactivate_provider
    from clawg_cli.config import save_env_value, load_config, save_config
    from clawg_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from clawg_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from clawg_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set CLAWG_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from clawg_cli.models import _PROVIDER_MODELS
...
def cmd_login(args):
    ...
    from clawg_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from clawg_cli.auth import logout_command
    logout_command(args)


def cmd_status(args):
    """Show status of all components."""
    from clawg_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from clawg_cli.cron import cron_command
    cron_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from clawg_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from clawg_cli.config import config_command
    config_command(args)
...
def cmd_version(args):
    ...
    try:
        from clawg_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall CLAWG."""
    from clawg_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def cmd_update(args):
    ...
    try:
        ...
        # Uses the PID file (scoped to CLAWG_HOME) to find this
...
"""
    ...
    def cmd_pairing(args):
        from clawg_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            from clawg_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from clawg_cli.skills_hub import skills_command
            skills_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            if clawg_bin:
                os.execvp(clawg_bin, ["clawg", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "clawg_cli.main", "--resume", selected_id],
                )
    ...
    def cmd_claw(args):
        from clawg_cli.claw import claw_command
        claw_command(args)
