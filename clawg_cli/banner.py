...
Pure display functions with no ClawgCLI state dependency.
"""
...
from clawg_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def check_for_updates() -> Optional[int]:
    ...
    ``~/.clawg/.update_check``).  Returns the number of commits behind,
    or ``None`` if the check fails or isn't applicable.
    """
    clawg_home = Path(os.getenv("CLAWG_HOME", Path.home() / ".clawg"))
    repo_dir = clawg_home / "clawg"
    cache_file = clawg_home / ".update_check"
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else CLAWG_CADUCEUS
    except Exception:
        _bskin = None
        _hero = CLAWG_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        ...
        if behind and behind > 0:
            ...
            right_lines.append(
                ...
                f"[dim yellow] — run [bold]clawg update[/bold] to update[/]"
            )
    except Exception:
    ...
    agent_name = _skin_branding("agent_name", "CLAWG")
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else CLAWG_AGENT_LOGO
        console.print(_logo)
        console.print()
