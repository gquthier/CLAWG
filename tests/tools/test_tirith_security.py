...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._clawg_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_clawg_bin(self):
        """After _INSTALL_FAILED, manual install in CLAWG_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        clawg_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(clawg_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(clawg_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._clawg_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == clawg_bin
            assert _tirith_mod._resolved_path == clawg_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# CLAWG_HOME isolation
# ---------------------------------------------------------------------------

class TestClawgHomeIsolation:
    def test_clawg_bin_dir_respects_clawg_home(self):
        """_clawg_bin_dir must use CLAWG_HOME, not hardcoded ~/.clawg."""
        from tools.tirith_security import _clawg_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"CLAWG_HOME": tmpdir}):
            result = _clawg_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_clawg_home(self):
        """_failure_marker_path must use CLAWG_HOME, not hardcoded ~/.clawg."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"CLAWG_HOME": "/custom/clawg"}):
            result = _failure_marker_path()
        assert result == "/custom/clawg/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets CLAWG_HOME; verify it's active."""
        clawg_home = os.getenv("CLAWG_HOME")
        assert clawg_home is not None, "CLAWG_HOME should be set by conftest"
        assert "clawg_test" in clawg_home, "Should point to test temp dir"

    def test_get_clawg_home_fallback(self):
        """Without CLAWG_HOME set, falls back to ~/.clawg."""
        from tools.tirith_security import _get_clawg_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove CLAWG_HOME entirely
            os.environ.pop("CLAWG_HOME", None)
            result = _get_clawg_home()
        assert result == os.path.join(os.path.expanduser("~"), ".clawg")
