"""Tests for clawg_cli configuration management."""
...
class TestGetClawgHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("CLAWG_HOME", None)
            home = get_clawg_home()
            assert home == Path.home() / ".clawg"

    def test_env_override(self):
        with patch.dict(os.environ, {"CLAWG_HOME": "/custom/path"}):
            home = get_clawg_home()
            assert home == Path("/custom/path")


class TestEnsureClawgHome:
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"CLAWG_HOME": str(tmp_path)}, clear=False):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from clawg_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
