...
from clawg_cli import config as clawg_config
...
def test_stash_local_changes_if_needed_returns_none_when_tree_clean(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(clawg_main.subprocess, "run", fake_run)

    stash_ref = clawg_main._stash_local_changes_if_needed(["git"], tmp_path)
...
def test_stash_local_changes_if_needed_returns_specific_stash_commit(monkeypatch, tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append((cmd, kwargs))
        if cmd[-2:] == ["status", "--porcelain"]:
            return SimpleNamespace(stdout=" M clawg_cli/main.py\n?? notes.txt\n", returncode=0)
...
def test_resolve_stash_selector_returns_matching_entry(monkeypatch, tmp_path):
    ...
    assert clawg_main._resolve_stash_selector(["git"], tmp_path, "abc123") == "stash@{1}"
...
def test_restore_stashed_changes_prompts_before_applying(monkeypatch, tmp_path, capsys):
    ...
    restored = clawg_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=True)
...
def test_restore_stashed_changes_applies_without_prompt_when_disabled(monkeypatch, tmp_path, capsys):
    ...
    restored = clawg_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=False)
...
def test_print_stash_cleanup_guidance_with_selector(capsys):
    clawg_main._print_stash_cleanup_guidance("abc123", "stash@{2}")
...
def test_restore_stashed_changes_exits_cleanly_when_apply_fails(monkeypatch, tmp_path, capsys):
    ...
    with pytest.raises(SystemExit, match="1"):
        clawg_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=True)
...
def test_stash_local_changes_if_needed_raises_when_stash_ref_missing(monkeypatch, tmp_path):
    ...
    with pytest.raises(CalledProcessError):
        clawg_main._stash_local_changes_if_needed(["git"], Path(tmp_path))
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(clawg_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(clawg_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(clawg_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(clawg_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(clawg_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(clawg_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(clawg_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
...
def test_cmd_update_tries_extras_first_then_falls_back(monkeypatch, tmp_path):
    ...
    clawg_main.cmd_update(SimpleNamespace())
