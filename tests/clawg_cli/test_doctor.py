...
import clawg_cli.doctor as doctor
import clawg_cli.gateway as gateway_cli
from clawg_cli import doctor as doctor_mod
from clawg_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "CLAWG_HOME", clawg_home)
    monkeypatch.delenv("CLAWG_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("CLAWG_INTERACTIVE")
        raise SystemExit(0)
