...
def test_sessions_delete_accepts_unique_id_prefix(monkeypatch, capsys):
    import clawg_cli.main as main_mod
    import clawg_state
    ...
    monkeypatch.setattr(clawg_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys,
        "argv",
        ["clawg", "sessions", "delete", "20260315_092437_c9a6", "--yes"],
    )
...
def test_sessions_delete_reports_not_found_when_prefix_is_unknown(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["clawg", "sessions", "delete", "missing-prefix", "--yes"],
    )
