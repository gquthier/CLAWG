"""
ClawgAgentBaseEnv -- Abstract Base Environment for clawg + Atropos

Provides the Atropos integration plumbing that all clawg environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via ClawgAgentLoop
...
# Ensure the clawg repo root is on sys.path so that imports like
...
# Load API keys from clawg/.env so all environments can access them
...
from environments.agent_loop import AgentResult, ClawgAgentLoop
from environments.tool_context import ToolContext

# Import clawg toolset infrastructure
...
class ClawgAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for clawg Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of clawg toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: clawg, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
...
class ClawgAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for clawg Atropos integration.
    ...
    name: Optional[str] = "clawg"
    env_config_cls = ClawgAgentEnvConfig

    def __init__(
        self,
        config: ClawgAgentEnvConfig,
        ...
        # Set terminal environment variables so clawg tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY clawg tool (terminal, file, web,
