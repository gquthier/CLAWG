...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs ClawgAgentLoop with terminal tool only
...
Requires: pip install clawg[yc-bench]
"""
...
class YCBenchEvalConfig(ClawgAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends ClawgAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(ClawgAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The ClawgAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs ClawgAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our ClawgAgentLoop.
