...
def _resolve_memory_mode(
    ...
    Object form:  { "default": "hybrid", "clawg": "honcho", ... }
...
class HonchoClientConfig:
    ...
    # Config object form: "memoryMode": { "default": "hybrid", "clawg": "honcho" }
    ...
    # Max chars of dialectic result to inject into clawg system prompt
    ...
    @classmethod
    def from_env(cls, workspace_id: str = "clawg") -> HonchoClientConfig:
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. clawg session title (from /title command)
          3. per-session strategy — clawg session_id ({timestamp}_{hex})
        ...
        # per-session: inherit clawg session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'clawg honcho setup' or set HONCHO_API_KEY. "
    ...
    if not resolved_base_url:
        try:
            ...
            clawg_cfg = load_config()
            honcho_cfg = clawg_cfg.get("honcho", {})
