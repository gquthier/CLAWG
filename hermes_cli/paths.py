...
- the app home directory (CLAWG_HOME with HERMES_HOME backward compatibility)
...
DEFAULT_LEGACY_HOME = Path.home() / ".hermes"
...
def get_runtime_home() -> Path:
    ...
    2) HERMES_HOME env var (backward compatibility)
    3) ~/.clawg when present
    4) ~/.hermes when present
    5) ~/.clawg default
    """
...
def sync_home_env_vars(home: Path | None = None) -> Path:
    """Keep CLAWG_HOME and HERMES_HOME aligned for runtime compatibility."""
    if home is None:
