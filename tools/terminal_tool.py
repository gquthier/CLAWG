...
def _check_disk_usage_warning():
    ...
    try:
        # Get total size of clawg directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "clawg-*")):
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    for failure in sudo_failures:
        if failure in output:
            return output + "\n\n💡 Tip: To enable sudo over messaging, add SUDO_PASSWORD to ~/.clawg/.env on the agent machine."
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (CLAWG_INTERACTIVE=1).
...
def _transform_sudo_command(command: str) -> tuple[str, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (CLAWG_INTERACTIVE=1):
    ...
    if not sudo_password:
        ...
        if os.getenv("CLAWG_INTERACTIVE"):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.clawg/.env or environment variables."
        )
...
def terminal_tool(
    ...
    try:
        ...
        if background:
            ...
            session_key = os.getenv("CLAWG_SESSION_KEY", "")
            ...
            try:
                ...
                if check_interval and background:
                    ...
                    watcher_platform = os.getenv("CLAWG_SESSION_PLATFORM", "")
                    watcher_chat_id = os.getenv("CLAWG_SESSION_CHAT_ID", "")
                    watcher_thread_id = os.getenv("CLAWG_SESSION_THREAD_ID", "")
...
def check_terminal_requirements() -> bool:
    ...
    Important: local and singularity backends now use clawg' own environment
    ...
    try:
        if env_type == "local":
            # Local execution uses clawg' own LocalEnvironment wrapper and does
...
if __name__ == "__main__":
    ...
    print(f"  TERMINAL_SANDBOX_DIR: {os.getenv('TERMINAL_SANDBOX_DIR', '~/.clawg/sandboxes')}")
