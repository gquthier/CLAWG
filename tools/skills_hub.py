#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the clawg Skills Hub.
...
Used by clawg_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            clawg_meta = metadata.get("clawg", {})
            if isinstance(clawg_meta, dict):
                tags = clawg_meta.get("tags", [])
        if not tags:
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                clawg_meta = meta_block.get("clawg", {})
                if isinstance(clawg_meta, dict):
                    tags = clawg_meta.get("tags", [])
